...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        padded_sentence = \
            [sentence[i] if i < len(sentence) else zero_vector for i in range(max_length)]
        padded_labels = [labels[i] if i < len(sentence) else zero_label for i in range(max_length)]
...
class RNNModel(NERModel):
    ...
    def add_placeholders(self):
        ...
        self.input_placeholder = tf.placeholder(
            tf.int32, (None, self.max_length, Config.n_features))
    ...
    def add_embedding(self):
        ...
        embeddings = tf.reshape(
            embeddings, (-1, self.max_length, self.config.n_features * Config.embed_size))
        ### END YOUR CODE
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        ### YOUR CODE HERE (~4-6 lines)
        U = tf.get_variable(
            'U', shape=(Config.hidden_size, Config.n_classes),
            initializer=tf.contrib.layers.xavier_initializer()
        )
        b2 = tf.get_variable(
            'b2', shape=(Config.n_classes,),
        ...
        with tf.variable_scope("RNN"):
            for time_step in range(self.max_length):
                ...
                inputs_batch = tf.slice(x, [0, time_step, 0], [-1, 1, -1])
                inputs_batch = tf.reshape(inputs_batch, [-1, Config.n_features * Config.embed_size])
                h, o = cell(inputs_batch, h, scope=tf.get_variable_scope())
                o_drop = tf.nn.dropout(o, dropout_rate)
                preds.append(tf.matmul(o_drop, U) + b2)  # element shape=(batch_size, n_classes)
                ### END YOUR CODE

        # Make sure to reshape @preds here.
        ### YOUR CODE HERE (~2-4 lines)
        preds = tf.stack(preds)  # shape=(max_length, batch_size, n_classes)
    ...
    def add_loss_op(self, preds):
        ...
        masked_loss = tf.boolean_mask(unmasked_loss, unmasked_loss)
