...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    for sentence, labels in data:
        ...
        for i in range(len(sentence)):
            windowed_input = sum(
                (
                    start if i + j < 0 else sentence[i + j] if i + j < len(sentence) else end
                    for j in range(-window_size, window_size + 1)
                ),
                []
            )
            windowed_data.append((windowed_input, labels[i]))
		### END YOUR CODE
...
class WindowModel(NERModel):
    ...
    def add_prediction_op(self):
        ...
        self.b2 = tf.get_variable('b2', shape=(self.config.n_classes,))

        self.h = tf.nn.relu(tf.matmul(x, self.W) + self.b1)
        self.h_drop = tf.nn.dropout(self.h, dropout_rate)
